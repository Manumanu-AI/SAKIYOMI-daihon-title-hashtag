...
def generate_response_with_llm_for_multiple_namespaces(index, user_input, namespaces):
    ...
    ユーザーが作成した台本の内容に適したタイトルとハッシュタグを生成してください。
    
    ----------
    # 情報一覧
    ユーザーが作成した台本{user_input}
    表紙タイトルDB {results_ns3}
    ----------
    # 依頼
    ①次の形式とルール1を厳守し、ユーザーが作成した台本に従って適切な魅力的なタイトルを5個出力してください。
    ②ルール2を厳守し、ユーザーが作成した台本の内容に適しているハッシュタグを30個出力してください。

    ##形式
    期待するアウトプットの型は以下の通りです。
    フック：
    ベース：
    ...
    - 名詞よりも動詞を使用し、キャッチーさと語呂の良さを意識してください
    - リーチ数が得られるタイトルや重要なキーワードを取り残さないようにしてください。
    - SEOを考慮してタイトルを考えてください。
    - フックとベースを合計して24文字以内に収めてください

    #ルール2
    - ユーザーが検索しそうで、エンゲージメントを促進してリーチ数が上がりやすくなるようなハッシュタグを生成する
    - 1ハッシュタグごとに改行してください
